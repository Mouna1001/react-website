...
class AddPost extends Component{ 
    componentDidUpdate(props,state){
        if(this.props.match.params.view === 'add' && this.props.admin.posts.filter(p=>p.title === this.props.values.title).lengths > 0 ){
    ...
    uploadImage=(e)=>{
        ...
        data.append('file',e.target.files[0],new Date().getTime() + e.target.files[0].name);
